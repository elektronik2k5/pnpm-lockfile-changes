...
Creates a comment inside Pull Request with the human-readable summary of the changes to the `pnpm-lock.yaml` file. Works in public and private repositories and offers a few customization options.
...
jobs:
  lockfile_changes:
    ...
    steps:
      ...
      - name: PNPM Lockfile Changes
        uses: elektronik2k5/pnpm-lockfile-changes@main
        with:
...
| `path`                 |         No         | `pnpm-lock.yaml` | Path to the `pnpm-lock.yaml` file in the repository. Default value points to the file at project root.           |
